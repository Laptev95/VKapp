...
namespace VkLib.Core.Groups
{
    ...
    public class VkGroupsRequest
    {
        ...
        public async Task<VkGroupById> GetById(long groupId, string fields = null)
        {
            var group = await GetById(new List<string>() { groupId.ToString() }, fields);
            if (group.Items != null && group.Items.Count > 0)
                return group.Items.First();

            return null;
        }
        public async Task<VkItemsResponse<VkGroupById>> GetById(IEnumerable<string> groupId, string fields)
        {
            if (_vkontakte.AccessToken == null || string.IsNullOrEmpty(_vkontakte.AccessToken.Token) || _vkontakte.AccessToken.HasExpired)
                throw new Exception("Access token is not valid.");
            ...
            if (groupId != null)
                parameters.Add("group_id", string.Join(",", groupId));
            ...
            var response = await new VkRequest(new Uri(VkConst.MethodBase + "groups.getById"), parameters).Execute();

            VkErrorProcessor.ProcessError(response);

            if (response.SelectToken("response") != null)
            {
                return new VkItemsResponse<VkGroupById>((from u in response["response"] select VkGroupById.FromJson(u)).ToList());
            }

            return VkItemsResponse<VkGroupById>.Empty;
        }
    }
}
